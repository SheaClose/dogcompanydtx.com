    ...
    <div class="Store-page-content-container">
      <div class="container">
        ...
        <div class="cart-container">
          ...
          <hr>
          <v-layout row wrap class="product-row" :key="i" v-for="(product,i) in cart">
            <v-flex xs3 sm1 md1 lg1>
              <img alt="product image" class="product-image" :src="product.product.imgUrl" />
            </v-flex>
            <v-flex class="product_title" xs3 sm1 md1 lg1>
              <div @click="reviewProduct(product.product)" class="">{{product.product.title}}</div>
            </v-flex>
            <v-flex xs3 sm3 md3 lg3 class="desc">
            ...
            <v-flex xs3 sm1 md1 lg1>
              <div class="center">
                <i @click="increaseQuantity(product.product._id)" class="material-icons pointer">add</i>
                {{product.quantity}}
                <i @click="decreaseQuantity(product.product._id)" class="material-icons pointer">remove</i>
              </div>
            </v-flex>
            <v-flex xs3 sm1 md1 lg1>
              <i @click="removeFromCart(product.product._id)" class="material-icons pointer">delete</i>
            </v-flex>
          </v-layout>
        </div>
...
export default {
  ...
  methods:{
    ...
    async fillCart(id){
      let {data: {cart}} = await this.$axios.get(`/api/cart/${id}`)
