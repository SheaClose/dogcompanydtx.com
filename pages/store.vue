      ...
      <div class="container">
        <div class="row">
          <div
            ...
            class="col s12 m12 l4 center product-repeat pointer prodrow"
          >
    ...
    <Modal v-if="showModal" @close="modalClose">
      <slot slot="body" id="modal1" class="modal modal-fixed-footer grey darken-4">
        <div class="modal-content grey darken-2">
          ...
          <p>You should recieve an e-mail shortly. Please hold on to this as confirmation of your order until you have received and are happy with your order. Should a problem arise, the Order Number will greatly assist us in locating your order.</p>
          <hr />

          <div>
            <span
              v-if="currentUserOrderInformation.total"
            >Order Total (before shipping): ${{currentUserOrderInformation.total}}</span>
            <br />
            <span
              v-if="currentUserOrderInformation.date"
            >Order Date: {{currentUserOrderInformation.date | formatDate}}</span>
            <div>
              <h5>Order:</h5>
            </div>
            <hr />
            <div>
              <ul :key="i" v-for="(cartItem, i) in currentUserOrderInformation.cart">
                ...
                <li
                  v-if="!cartItem.product.category !== 'apparel'"
                >Color: {{cartItem.product.color}}</li>
            ...
            <div>
              <ul>
                ...
                <span
                  v-if="currentUserOrderInformation.user.nonUSAddress"
                >Adress: {{currentUserOrderInformation.user.nonUSAddress}}</span>
                <span
                  v-else
                >Adress: {{currentUserOrderInformation.user.street}}, {{currentUserOrderInformation.user.city}}, {{currentUserOrderInformation.user.state}} {{currentUserOrderInformation.user.zipcode}}</span>
...
export default {
  ...
  async mounted() {
    ...
    let products = data
      ...
      .filter(c => c);
    this.products = [
      ...products.filter(c => c.category == "apparel"),
      ...products.filter(c => c.category == "merch"),
      ...products.filter(c => c.category == "bundle")
    ];
