  ...
  function($scope, $state, storeService) {
    ...
    angular.element(document).ready(() => {
      ...
      win.on('scroll', function() {
        var x = win.scrollTop();
        $('.Store-page-container').css('background-size', 125 + parseInt(x / 6) + 'vh');
        $('.Store-page-content-container').css('background-color', 'rgba(0,0,0, ' + retY(win) + ')');
      });
      storeService.getAllProducts().then(response => {
        let products = response.data
          .reduce((acc, cur) => (!acc.includes(cur.title) ? [...acc, cur.title] : acc), [])
          .map(c => {
            var avail = response.data.find(product => c == product.title && product.available);
